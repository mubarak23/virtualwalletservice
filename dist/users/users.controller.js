...
let UsersController = class UsersController {
    ...
    async create(res, createUserDto) {
        try {
            const { token, user, wallet } = await this.usersService.createUser(createUserDto);
            return res.status(common_1.HttpStatus.CREATED).json({ token, user, wallet });
        }
        catch (error) {
            return res.status(common_1.HttpStatus.FORBIDDEN).json(error);
        }
    }
    async loginUser(res, loginAuthDto) {
        try {
            const { token, user, wallet } = await this.usersService.loginUser(loginAuthDto);
